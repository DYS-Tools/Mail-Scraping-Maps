...
class simple_html_dom_node
{
  ...
  const HDOM_SMARTY_AS_TEXT= 1;
	...
	function dump_node($echo = true)
	{
		...
		if (isset($this->node->_[self::HDOM_INFO_INNER])) {
			$string .= "'" . $this->node->_[self::HDOM_INFO_INNER] . "'";
		} else {
			$string .= ' NULL ';
		}
  ...
  const HDOM_SMARTY_AS_TEXT= 1;
	...
	protected function parse()
	{
		while (true) {
			...
			$this->node = new simple_html_dom_node($this);
			++$this->cursor;
			$this->node->_[self::HDOM_INFO_TEXT] = $s;
			$this->link_nodes($this->node, false);
		}
	}
